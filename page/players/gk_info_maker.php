...
$tpl->loadTemplatefile("gk_info.tpl.php", true, true); 
...
if (!$rs) {
    ...
    if ($rs->RecordCount() > 0) {
		...
		$tpl->setVariable("PLAYER_OR_GK", "goal keeper");
		...
		// Physical
		$tpl->setVariable("AGILITY", $rs->fields['agility']);
		$tpl->setVariable("REFLEX", $rs->fields['reflex']);  
		$tpl->setVariable("HEIGHT", $rs->fields['height']);
		// Technical
		$tpl->setVariable("HANDING", $rs->fields['handing']);
		$tpl->setVariable("RUSHING_OUT", $rs->fields['rushing_out']);
		$tpl->setVariable("POSITIONING", $rs->fields['positioning']);
		$tpl->setVariable("AERIAL_ABILITY", $rs->fields['aerial_ability']);
		// Mental
		$tpl->setVariable("JUDGMENT", $rs->fields['judgment']);
