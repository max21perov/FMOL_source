...
$tpl->loadTemplatefile('search_result.tpl.php', true, true); 
// pager
...
$query = sprintf(
			" SELECT t.team_id AS primary_team_id, t.team_id, t.name AS team_name, " . 
			" n.name AS nation_name " . 
			" FROM team t, club c, nation n " . 
			" WHERE t.club_id=c.club_id AND c.nation_id=n.nation_id " . 
			" AND t.team_id like '%s' " ,
			$team_id);
...
if (!$rs) {
    ...
    for (; !$rs->EOF; $rs->MoveNext(), $index++) {
		...
		$tpl->setCurrentBlock('search_result') ;
		if ($index % 2 != 0 )
			$tpl->setVariable("SEARCH_RESULT_TR_CLASS", 'gSGRowEven') ;
		else 
			$tpl->setVariable("SEARCH_RESULT_TR_CLASS", 'gSGRowOdd') ;

		$tpl->setVariable('PRIMARY_TEAM_ID', $rs->fields['primary_team_id']) ;
		...
		$tpl->parseCurrentBlock('search_result') ;
 ...
 * pager
 */
$tpl_pager->setVariable("FORM_ACTION", "/fmol/page/search/search_result.php") ;
$tpl_pager->setVariable("FILTER_NAME", "team_id"); 
$tpl_pager->setVariable("FILTER_VALUE", $team_id);
...
// pre page
if (!$rs->AtFirstPage()) {
	$tpl_pager->setVariable("PRE_PAGE_URL", "<a href=$PHPSELF?next_page=". ($rs->AbsolutePage() - 1). "&team_id=$team_id>pre</a>") ;
}
...
// next page
if (!$rs->AtLastPage()) {
	$tpl_pager->setVariable("NEXT_PAGE_URL", "<a href=$PHPSELF?next_page=". ($rs->AbsolutePage() + 1). "&team_id=$team_id>next</a>") ;
}
