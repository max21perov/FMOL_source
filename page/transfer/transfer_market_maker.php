...
$tpl->loadTemplatefile("transfer_market.tpl.php", true, true); 
// pager
...
$query = sprintf(
			...
			" player.position, tran.start_price, tran.bids, tran.start_time " . 
			" FROM transfer_list tran, team, player " . 
			" WHERE tran.player_id=player.player_id " . 
			" AND tran.team_id=team.team_id " . 
			" %s " . 
			" ORDER BY tran.start_time DESC " ,
			$sql_filter);  
...
if (!$rs) {
    ...
    for (; !$rs->EOF; $rs->MoveNext(), $index++) {
		$tpl->setCurrentBlock('transfer_market') ;
		...
		$tpl->setVariable("START_PRICE", number_format($rs->fields['start_price'], null, null, ",")) ;
		...
		$tpl->parseCurrentBlock('transfer_market') ;
 ...
 * pager
 */
$tpl_pager->setVariable("FORM_ACTION", "/fmol/page/transfer/transfer_market.php") ;
