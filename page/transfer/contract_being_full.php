        ...
        <td height="25" ><table width="100%" height="100%"  border="0" cellpadding="0" cellspacing="0">
          <tr class="MenuText">
			...
			<!-- InstanceBeginEditable name="EditNavigation" -->
	 		...
	 		<td width="120" align="center"><a href="<?=$_SERVER['REQUEST_URI']?>">Free Market</a></td>
            <td>&nbsp;</td>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td><table width="100%" cellspacing="0" class="TableMenu" >
              <tr>
				...
				<td width="20%" class="ButtonSel">Contract Being Full</td>
        ...
        <tr>
		  ...
		  <?php 
		  	...
		  	$query  = " SELECT value ";
			$query .= " FROM system_const ";
			$query .= " WHERE name='season_period' ";  
			$rs = &$db->Execute($query);
			if ($rs && $rs->RecordCount() > 0 ) {  
				if (doubleval($rs->fields['value']) == 0.5) {
					require_once ('contract_being_full_maker.php'); 
				}
				else {
					require_once ('contract_being_full_closed.php'); 
				}
			}
			else { 
				require_once ('contract_being_full_closed.php'); 
