...
$tpl->loadTemplatefile("invite_join_free_signing.tpl.php", true, true); 
...
$query = sprintf(
			...
			" p.age, p.player_value, p.player_or_gk, fm.update_seasons " . 
			" FROM player p " . 
			" LEFT JOIN free_market fm ON fm.player_id=p.player_id " . 
			" WHERE p.player_id='%s' " ,
			$player_id);
...
if (!$rs) {
    ...
    if ($rs->RecordCount() > 0) {
		...
		$tpl->setVariable("STAY_SEASONS", $rs->fields['update_seasons']) ;		
		
		$full_salary = doubleval($rs->fields['player_value'])*0.2;
		$salary_percent = 0.5;
		switch (intval($rs->fields['update_seasons'])) {
		case 1: // first season 100%
			$salary_percent = 1.0;
			break;
		case 2: // second season 75%
			$salary_percent = 0.75;
			break;
		default: // other 50%
			$salary_percent = 0.5;
		...
		$tpl->setVariable("RETURN_PAGE_URL", "/fmol/page/transfer/free_market.php");
