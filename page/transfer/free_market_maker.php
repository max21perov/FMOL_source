...
$tpl->loadTemplatefile("free_market.tpl.php", true, true); 
// pager
...
$query = sprintf(
			" SELECT player.player_id, player.custom_given_name AS given_name, " . 
			" player.custom_family_name AS family_name, player.highest_tsp, player.age, " . 
			" player.position, fm.update_seasons, fm.bids, fm.start_time " . 
			" FROM free_market fm, player " . 
			" WHERE fm.player_id=player.player_id " . 
			" %s " . 
...
if (!$rs) {
    ...
    for (; !$rs->EOF; $rs->MoveNext(), $index++) {
		$tpl->setCurrentBlock('free_market') ;
		...
		$tpl->parseCurrentBlock('free_market') ;
