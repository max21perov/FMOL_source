...
$tpl->loadTemplatefile("transfer_hotlist.tpl.php", true, true); 
// pager
...
$query = sprintf(
			" SELECT hl.id AS hotlist_id, team.team_id, team.name AS team_name, " . 
			...
			" player.position " . 
			" FROM hot_list hl, player " . 
			" LEFT JOIN team ON player.team_id=team.team_id " .
			" WHERE hl.player_id=player.player_id " . 
			" AND hl.team_id='%s' " . 
			" AND hl.team_id<>player.team_id " . 
			" %s " ,
...
if (!$rs) {
    ...
    for (; !$rs->EOF; $rs->MoveNext(), $index++) {
		$tpl->setCurrentBlock('transfer_hotlist') ;
		
		$tpl->setVariable("TEAM_ID", $rs->fields['team_id']) ;
		if ($rs->fields['team_name'] == "") {
			$tpl->setVariable("TEAM_NAME", "Free Market") ;		
			$tpl->setVariable("TEAM_NAME_ONLY_DISPLAY", "block") ;		
			$tpl->setVariable("TEAM_NAME_LINK_DISPLAY", "none") ;	
		}
		else {
			$tpl->setVariable("TEAM_NAME", $rs->fields['team_name']) ;
			$tpl->setVariable("TEAM_NAME_ONLY_DISPLAY", "none") ;		
			$tpl->setVariable("TEAM_NAME_LINK_DISPLAY", "block") ;	
		}
		...
		$tpl->setVariable("HOTLIST_ID", $rs->fields["hotlist_id"]) ;
		
		$tpl->parseCurrentBlock('transfer_hotlist') ;
 ...
 * pager
 */
$tpl_pager->setVariable("FORM_ACTION", "/fmol/page/transfer/transfer_hotlist.php") ;
